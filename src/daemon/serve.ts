// Headless HTTP add API: grab exposes a tiny local server so another program
...
function log(message: string): void {
  console.log(`[grab serve] ${new Date().toISOString()} ${message}`);
}
  ...
  if (!LOOPBACK_HOSTS.has(host) && !token) {
    console.error(
      `error: refusing to bind ${host} without a token. Pass --token <secret> ` +
        `(or set GRAB_API_TOKEN or TORLINK_API_TOKEN), or bind 127.0.0.1.`,
    );
    process.exit(1);
    return;
  }
